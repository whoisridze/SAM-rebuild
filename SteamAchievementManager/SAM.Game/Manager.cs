...
namespace SAM.Game
{
    internal partial class Manager : Form
    {
        ...
        private async Task<int> StoreAchievementsWithDelayAsync(int minDelaySec, int maxDelaySec, CancellationToken cancellationToken)
        {
            if (this._AchievementListView.Items.Count == 0)
            {
                return 0;
        ...
        private async void OnStore(object sender, EventArgs e)
        {
            // Check if random delay mode is enabled
            bool useRandomDelay = this._UseRandomDelayCheckBox.Checked;
            int achievements;

            if (useRandomDelay)
            {
                // Parse delay values
                if (!int.TryParse(this._MinDelayTextBox.Text, out int minDelay) || minDelay < 0)
                {
                    MessageBox.Show(
                        this,
                        "Invalid minimum delay value. Please enter a non-negative integer.",
                        "Error",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    return;
                ...
                if (!int.TryParse(this._MaxDelayTextBox.Text, out int maxDelay) || maxDelay < 0)
                {
                    MessageBox.Show(
                        this,
                        "Invalid maximum delay value. Please enter a non-negative integer.",
                        "Error",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                    return;
                ...
                try
                {
                    ...
                    achievements = await this.StoreAchievementsWithDelayAsync(minDelay, maxDelay, cts.Token);
                }
                catch (OperationCanceledException)
                {
                    this._GameStatusLabel.Text = "Operation cancelled.";
                    this.EnableInput();
                    return;
                }
                catch (Exception ex)
            ...
            {
                achievements = this.StoreAchievements();
            }
